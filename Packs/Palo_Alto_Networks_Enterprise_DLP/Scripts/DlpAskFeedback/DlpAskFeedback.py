...
def create_blocks(file_name: str, policy_name: str, entitlement: str, options: list, reply: str) -> list:
    value = json.dumps({
    ...
    blocks: list = [{
        ...
        'text': {
            'type': 'mrkdwn',
            'text': FILE_INFO.substitute(file_name=file_name)
        }
    },
        {
            'type': 'section',
            'text': {
                'type': 'mrkdwn',
                'text': POLICY_INFO.substitute(policy_name=policy_name)
            }
    },
        {
            "type": "section",
...
def main():
    ...
    blocks = json.dumps(create_blocks('dummy_file.doc', 'dummy policy', entitlement_string, OPTIONS, reply))
